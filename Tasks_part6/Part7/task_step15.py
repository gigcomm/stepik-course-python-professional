...
# чтобы он вывел все расширения файлов, присутствующие в списке files, указав для каждого количество файлов с данным расширением.
...
expansion = [file.split('.')[-1] for file in files]

count_files = Counter(expansion)

for name_file, count in sorted(count_files.items()):
    print(f"{name_file}: {count}")