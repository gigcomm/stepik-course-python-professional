...
#
# Дополните приведенный ниже код, чтобы он сгруппировал сотрудников по соответствующим отделам и
# вывел названия всех отделов, указав для каждого имена и фамилии его сотрудников.
...
for post, name in staff_broken:
    info[post].add(name)

for post in sorted(info.keys()):
    employees = sorted(info[post])
    print(f"{post}: {employees}")
